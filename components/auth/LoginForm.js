		...
		<div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col">
			<form onSubmit={props.onClick}>
				...
				<div className="mb-6">
					...
					{props.passwordError ? props.passwordError.map((erro, index) =>
						<p key={index} className="text-red-500 text-xs italic">{erro}</p>
					) : null}
					{props.nonFieldError ? props.nonFieldError.map((erro, index) =>
						<p key={index} className="text-red-500 text-xs italic">{erro}</p>
					) : null}
